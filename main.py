...
from api.formatter import transform_delta_produtos_to_json
from database.operations import get_delta_products, insert_snapshot_product
...
def main() -> None:
    ...
    try:
        ...
        with Session(cic_engine, expire_on_commit=False) as session:  # Desativa expiração após commit
            for batch in get_delta_products(session, batch_size=1000):
                logger.info(f"[N/A] Iniciando processamento de lote com limite de {len(batch)} produtos.")
                for produto in batch:
                    try:
                        codigoitem = produto.codigoitem  # Acesso ao atributo pode falhar
                    except ObjectDeletedError as e:
                        logger.warning(f"[N/A] Erro: Objeto DeltaProdutos deletado ou não presente: {str(e)}")
                        has_errors = False
                        continue
                    except Exception as e:
                        logger.error(f"[N/A] Erro ao acessar codigoitem do produto: {str(e)}")
                        has_errors = True
                        continue
                    logger.info(f"[{codigoitem}] Iniciando processamento do produto.")
                    try:
                        product_json = transform_delta_produtos_to_json(produto, codigoitem)
                        logger.info(f"[{codigoitem}] Conversão para JSON concluida com sucesso")
                        try:
                            success, response_text = post_to_api(product_json, codigoitem)
                        except Exception as e:
                        ...
                        if success:
                            ...
                            try:
                                insert_snapshot_product(session, produto, codigoitem)
                            except Exception as e:
                            ...
                            try:
                                ...
                                if any(error.get("type") == "ExistConfig" and error.get("message") == "Code exist" for error in errors):
                                    ...
                                    try:
                                        success, response_text = put_to_api(product_json, codigoitem)
                                        if success:
                        ...
                        logger.error(f"[{codigoitem}] Erro: Objeto DeltaProdutos deletado ou não presente durante processamento: {str(e)}")
                        has_errors = True
                        continue
                    except Exception as e:
                        logger.error(f"[{codigoitem}] Erro ao processar produto: {str(e)}")
                        has_errors = True
                        continue
        ...
        logger.info("[N/A] Finalizando process_products com sucesso")
    except Exception as e:
        logger.error(f"[N/A] Erro em process_products: {str(e)}")
        raise
